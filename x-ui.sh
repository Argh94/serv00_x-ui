...
reset_config() {
    ...
    echo -e "All panel settings have been reset to default values. Please restart the panel and access it using the default port ${green}12345${plain}."
    confirm_restart
}
...
set_port() {
    echo && echo -n -e "Enter port number [1-12345]: " && read -r port
    if [[ -z "${port}" ]]; then
        LOGD "Canceled"
        before_show_menu
    else
...
set_traffic_port() {
    echo && echo -n -e "Enter traffic monitoring port number [1-12345]: " && read -r trafficport
