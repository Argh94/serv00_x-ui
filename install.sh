...
wget -q --no-check-certificate -O "$xray_bin" "https://github.com/XTLS/Xray-core/releases/download/v${xray_version}/Xray-freebsd-${arch}.zip"
if [[ $? -ne 0 ]]; then
...
unzip -o "$xray_bin" -d "$config_dir" && rm "$xray_bin.zip"
if [[ $? -ne 0 ]]; then
